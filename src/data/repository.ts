...
export class Repository {
  ...
  async saveMod(mod: Mod, now = Date.now(), schema = CURRENT_SCHEMA): Promise<ModRecord> {
    ...
    return this.tx(['mods', 'textures', 'history'], 'readwrite', async (t) => {
      ...
      const record: ModRecord = { mod: stored, revision: (previous?.revision ?? 0) + 1, updatedAt: now, deletedAt: null, schema: Math.max(schema, previous?.schema ?? 0) }
  ...
  async saveMods(mods: Mod[], now = Date.now()) {
    ...
    return this.tx(['mods', 'textures', 'history'], 'readwrite', async (t) => {
      ...
      for (let i = 0; i < mods.length; i++) {
        ...
        const record: ModRecord = { mod: stored[i], revision: (previous?.revision ?? 0) + 1, updatedAt: now, deletedAt: null, schema: CURRENT_SCHEMA }
        await put(t, 'mods', record)
        records.push(record)
      }
