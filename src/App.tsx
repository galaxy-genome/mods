...
import { QuickBuildPage } from '@/features/quickbuild/QuickBuildPage'
...
/** A screen a mod doesn't have opens its contents rather than an empty shell. */
function ModContents() {
  const { modId } = useParams()
  return <Navigate to={`/mod/${splitViewId(modId ?? '').modId}`} state={{ contents: true }} replace />
}
    ...
    <>
      <Routes location={background ?? location}>
        ...
        <Route path="/mod/:modId" element={<ModShell />}>
          ...
          <Route path="build" element={<QuickBuildPage />} />
