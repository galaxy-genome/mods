...
export interface EditorState {
  ...
  mods: Mod[]
  /** Every quest and stars part of every mod (derived from mods). */
  parts: ModPart[]
...
const withParts = (s: Omit<EditorState, 'parts'> & { parts?: ModPart[] }): EditorState => ({ ...s, parts: s.mods.flatMap(partsOf) })
...
function emit(next: Omit<EditorState, 'parts'> & { parts?: ModPart[] }) {
  state = next.mods === state.mods && next.parts ? (next as EditorState) : withParts(next)
...
export function useMod(id: string | undefined) {
  return useEditor((s) => s.mods.find((b) => b.meta.id === (id ? splitViewId(id).modId : undefined)))
}
...
function readOnlyToast(modId: string) {
  toast(t('lib.readOnly'), { description: t('lib.readOnlyHelp'), action: { label: t('lib.makeCopy'), onClick: () => { const cid = duplicateMod(modId); if (cid) location.assign(`/mod/${cid}`) } } })
}
...
/** The default mods: the samples, plus library entries marked default with their texture sheets. */
async function defaultMods() {
  return [...samples(), ...await Promise.all(COMMUNITY.filter((e) => e.role !== 'library').map(async (e) => entryMod(e, await loadEntryTextures(e).catch(() => []))))]
}
...
export function duplicateMod(id: string) {
  ...
  if (isReadOnly(source) || source.meta.community) {
    ...
    if (source.meta.favorite) setFavorite([modId], false)
  }
  addMod(copy)
...
export const hasUnmodifiedCopy = (entryId: string) => state.mods.some((b) => b.meta.community?.entryId === entryId && !b.meta.modified)
...
export async function addCommunityEntry(entryId: string, favorite = false) {
  ...
  if (!entry || hasUnmodifiedCopy(entryId)) return
