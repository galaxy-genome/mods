...
import { compassOf, distanceLy } from '@/features/map/places'
import { useT } from '@/i18n'
import {
  BEHAVIOURS, GAME_QUEST_ID_MAX, GAME_QUEST_ID_MIN, gameQuest, GOODS, PORTRAIT_GROUPS, SHIP_MODELS, STATION_FACTIONS, humanize,
} from '@/lib/reference'
...
import { FactionDot, GoodsIcon, OptionRow, PortraitTile, RecentRow, ShipSilhouette, factionName, matches, useRecent } from './common'
...
export function PlacePicker({ open, onOpenChange, nested, kind, value, onSelect, system, title, allowOwnStation, generated }: Base & {
  ...
  system?: string
  title?: string
  /** Pinned first for station-menu conditions: "Your own station" returns "OWN". */
  ...
  const where = (o: PlaceOption) => {
    const at = kind === 'station' ? resolve?.station(o.name) : o.x != null && o.y != null ? { x: o.x, y: o.y, system: o.name } : resolve?.system(o.name)
    return at ? placeLine({ ly: distanceLy(at, { x: 0, y: 0 }), direction: compassOf(at) }) : null
  }
  ...
  const row = (o: PlaceOption, onPick: () => void, badge?: string) => (
    <OptionRow key={`${o.mod?.meta.id ?? ''}:${o.name}`} navKey={`pick:${o.name}`} selected={value === o.name} onClick={onPick} trailing={kind !== 'planet' ? mapLink(o.system ?? o.name) : undefined}>
      <span className="flex items-center gap-2">
        ...
        <span className={cn('truncate text-[15px]', value === o.name ? 'text-cyan' : 'text-white')}>{kind === 'station' ? stationLabel(o.system, o.name) : o.name}</span>
        {badge && <Badge tone="cyan">{badge}</Badge>}
      </span>
      <span className="block font-mono text-[11px] leading-snug text-dim">
        {kind === 'system' ? [o.security ?? t('pickers.unknownSecurity'), where(o)].filter(Boolean).join(' · ')
          : kind === 'station' ? where(o) ?? factionName(factionKey(o.faction)) : humanize(o.type ?? '')}
      </span>
    </OptionRow>
  )
    ...
    <Sheet open={open} onOpenChange={onOpenChange} nested={nested} title={heading} full
      ...
      <div>
        <div className="sticky top-0 z-10 space-y-2 bg-deep pb-2">
          ...
          {kind === 'station' && (
            <div className="scrollbar-none flex gap-2 overflow-x-auto pb-1" role="group" aria-label={t('pickers.faction')}>
...
export function QuestPicker({ open, onOpenChange, nested, value, onChange, excludeModId }: Base & { value: number[]; onChange: (ids: number[]) => void; excludeModId?: string }) {
  ...
  const gameName = (id: number) => gameQuest(id)?.name ?? t('pickers.gameQuestN', { id })
  ...
  const game = gameIds.map((id) => ({ id, g: gameQuest(id) })).filter(({ id, g }) => matches(q, gameName(id), id, g?.charName, g?.station))
