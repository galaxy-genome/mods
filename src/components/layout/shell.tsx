...
import { ReadOnlyFields } from '@/components/ui/inputs'
...
function Inspector({ modId }: { modId: string }) {
  const t = useT()
  const [tab, setTab] = React.useState<'help' | 'problems' | null>(null)
  const field = useFocusedField()
  const { errors, warnings } = useProblems(modId)
  const n = errors.length + warnings.length
  // Opens on whatever needs attention: problems when the mod has any, help otherwise. A choice sticks.
  const shown = tab ?? (n ? 'problems' : 'help')
  return (
    <aside aria-label={t('shell.inspector')} className="sticky top-0 flex h-dvh w-[320px] shrink-0 flex-col border-l border-edge bg-deep">
      <div role="tablist" className="grid min-h-14 grid-cols-2 border-b border-edge px-2">
        {(['help', 'problems'] as const).map((k) => (
          <button
            ...
            onClick={() => setTab(k)}
            className={cn('section-label flex items-center justify-center gap-2 border-b-2 !text-[12px]', shown === k ? 'border-cyan !text-white' : 'border-transparent hover:!text-ink')}
          >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
        {shown === 'problems' ? (
          <ProblemsList modId={modId} active={shown === 'problems'} />
        ) : field ? (
...
export function ModShell() {
  ...
  const menu = [
    ...
    ...(isQuest ? [{ label: t('shell.viewJson'), icon: <Braces />, onSelect: () => navigate(`${base}/json`) }] : []),
  ...
  return (
    <HeaderContext.Provider value={setHeader}>
      <ReadOnlyFields.Provider value={readOnly}>
          ...
          <aside className="sticky top-0 flex h-dvh w-[240px] shrink-0 flex-col gap-6 border-r border-edge bg-deep px-3 py-4">
            ...
            <div className="flex flex-col gap-2">
              <div className="flex flex-col gap-0.5 px-3" title={mod.meta.title}>
                ...
                {multiPart && <span className="truncate font-mono text-[11px] text-dim">{owner.meta.title}</span>}
              </div>
              <nav className="flex flex-col gap-1">
              {tabs.map((tb) => {
                const count = tabCount(mod, tb.path)
                return (
                  <NavLink key={tb.path} to={tabTo(tb.path)} className={({ isActive }) => cn('flex h-11 items-center gap-3 rounded-[2px] px-3 text-[14px]', isActive ? 'bg-cyan/10 text-cyan' : 'text-ink hover:bg-white/5')}>
            ...
            {tablet && !hideTabs && (
              <nav aria-label={t('shell.modSections')} className="flex justify-center border-b border-edge bg-deep/95 px-4 py-2 backdrop-blur">
                <div className="flex w-full max-w-[640px] rounded-[2px] border border-edge">
                  {tabs.map((tb) => (
                    <NavLink key={tb.path} to={tabTo(tb.path)} className={({ isActive }) => cn('flex h-11 flex-1 items-center justify-center gap-2 border-r border-edge text-[13px] last:border-r-0', isActive ? 'bg-cyan/10 text-cyan' : 'text-ink hover:bg-white/5')}>
      ...
      {!desktop && !tablet && !hideTabs && (
        <nav aria-label={t('shell.modSections')} className="fixed inset-x-0 bottom-0 z-30 border-t border-edge bg-deep/95 backdrop-blur safe-bottom">
          <div className="grid" style={{ gridTemplateColumns: `repeat(${tabs.length}, minmax(0, 1fr))` }}>
            {tabs.map((tb) => (
              <NavLink key={tb.path} to={tabTo(tb.path)} className={({ isActive }) => cn('flex h-[60px] flex-col items-center justify-center gap-1 text-[11px]', isActive ? 'text-cyan' : 'text-dim')}>
                <TabIcon name={tb.icon} className="size-5" />
                {t(tb.label)}
              </NavLink>
            ))}
