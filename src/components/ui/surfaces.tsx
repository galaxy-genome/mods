...
export function Card({ className, children, tone, ...props }: React.HTMLAttributes<HTMLDivElement> & { tone?: 'cyan' | 'amber' | 'danger' | 'success' }) {
  return (
    <div
      className={cn(
...
export function ListRow({
  icon, title, subtitle, value, onClick, chevron = true, className, trailing, muted, fieldKey, href,
}: {
  ...
  if (href) return <a data-field={fieldKey} href={href} className={cls}>{inner}</a>
  if (onClick) return <button data-field={fieldKey} type="button" onClick={onClick} className={cls}>{inner}</button>
...
export function SentenceCard({ icon, children, onClick, tone, trailing, className, fieldKey }: {
