...
/** True inside a read-only mod: text fields show their values and take no typing. */
export const ReadOnlyFields = React.createContext(false)
...
export const Input = React.forwardRef<HTMLInputElement, React.InputHTMLAttributes<HTMLInputElement> & { invalid?: boolean; warn?: boolean }>(
  ({ className, invalid, warn, ...props }, ref) => (
    <input ref={ref} readOnly={React.useContext(ReadOnlyFields)} aria-invalid={invalid || undefined} data-warn={warn || undefined} className={cn(fieldBase, 'h-11', className)} {...props} />
  ),
)
...
export function SearchInput({ value, onChange, placeholder, autoFocus, className }: {
  ...
  return (
    <div className={cn('relative', className)}>
      ...
      <Input value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} autoFocus={autoFocus} className="pl-9 pr-10" type="text" readOnly={false} inputMode="search" enterKeyHint="search" />
      {value && (
  ...
  return (
    <div className="relative">
      <textarea
        ...
        ref={ref}
        rows={rows}
        readOnly={React.useContext(ReadOnlyFields)}
        value={value}
...
/** Slider positions: 1 ly steps up to 100 ly, then 5 ly steps. */
const radiusAt = (pos: number) => (pos < 100 ? pos : 100 + (pos - 100) * 5)
const positionOf = (r: number) => (r < 100 ? r : 100 + Math.round((r - 100) / 5))

/** Labelled light-year radius slider with the value shown. */
export function RadiusSlider({ label, value, min = 1, max, onChange }: { label: string; value: number; min?: number; max: number; onChange: (r: number) => void }) {
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-baseline justify-between gap-2">
        <span className="text-[14px] text-ink">{label}</span>
        <span className="font-mono text-[13px] text-cyan">{t('quickbuild.lyValue', { n: value })}</span>
      </div>
      <Slider ariaLabel={label} value={[positionOf(value)]} min={positionOf(min)} max={positionOf(max)} step={1} tone="amber" onChange={([pos]) => onChange(radiusAt(pos))} />
    </div>
  )
}
