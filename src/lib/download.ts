...
import { GAME_QUESTS, GAME_QUEST_ID_MAX, GAME_QUEST_ID_MIN, LANGS } from './reference'
...
export function planDownload(favorites: Mod[], lang: Lang, all: ModPart[], mods: Mod[] = favorites): DownloadPlan {
  ...
  for (const q of plan.quests) {
    ...
    for (const id of q.content.settings.requiredQuestIds) {
      if (id === 0 || (id >= GAME_QUEST_ID_MIN && id <= GAME_QUEST_ID_MAX) || GAME_QUESTS.some((g) => g.id === id) || plan.quests.some((o) => o.questId === id)) continue
