...
export function questProblems(mod: QuestView, all: ModPart[], galaxy = getGalaxy()): Problem[] {
  ...
  if (mod.meta.origin !== 'game' && s.questId >= GAME_QUEST_ID_MIN && s.questId <= GAME_QUEST_ID_MAX) {
    const g = gameQuest(s.questId)
  ...
  q.steps.forEach((step, i) => {
    ...
    step.dialogue.forEach((line, li) => {
      ...
      // The game falls back to the quest's contact (DialogBoard.as:218), so an empty speaker still speaks as someone.
      if (!line.speaker.trim()) add('warning', `speaker-${line.id}`, t('rules.noSpeaker', { name: s.charName || t('rules.theContact') }), linePath, lineLabel, 'speaker',
        s.charName.trim() ? { label: t('rules.useContactName'), apply: () => updateQuest(mod.meta.id, (d) => {
          const ln = d.steps.flatMap((st) => st.dialogue).find((x) => x.id === line.id); if (ln) ln.speaker = s.charName
        }) } : undefined)
  ...
  // The mod's own stars remove or change a generated system this quest uses.
  const own = galaxy && sourcesOf(all).find((m) => m.meta.id === splitViewId(mod.meta.id).modId)
  if (own?.stars) {
    const { changes } = modDeps(own, { galaxy })
    for (const u of questPlaceUses(q)) {
      const key = changes.deletedGenerated.includes(u.name) ? 'lib.issueGeneratedDeleted' : changes.renumberedGenerated.includes(u.name) ? 'lib.issueGeneratedChanged' : null
      if (key) add('error', `generated-${u.path}-${u.field}-${u.name}`, t(key, { a: mod.meta.title, b: own.meta.title, system: u.name }), u.path, u.name, u.field)
    }
  }
