...
const has = (src: PlaceSource, kind: PlaceUse['kind'], name: string): boolean => {
  if (kind === 'quest') return src.quests.some((q) => String(q.versions[q.primaryLang]?.settings.questId) === name)
  const s = src.stars
  if (!s) return false
  if (kind === 'system') return s.stars.some((x) => x.name === name)
  if (kind === 'station') return s.stations.some((x) => x.name === name)
  if (kind === 'planet') return s.planets.some((x) => x.name === name)
  return has(src, 'system', name) || has(src, 'station', name) || has(src, 'planet', name)
}
