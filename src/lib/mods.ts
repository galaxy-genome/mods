...
export function modFromParts(views: ModPart[], meta?: Partial<ModMeta>, textures: TexturePart[] = []): Mod {
  ...
  for (const v of views) {
    ...
    // The game's quest id names the part, so the same files always build the same part ids and links to them travel.
    const id = String(content.settings.questId ?? uid())
    b.quests.push({ id: b.quests.some((x) => x.id === id) ? uid() : id, primaryLang: q.primaryLang, versions: { ...q.versions } })
  }
