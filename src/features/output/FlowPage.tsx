...
import { GAME_QUESTS } from '@/lib/reference'
...
function Graph({ q, modId }: { q: QuestContent; modId: string }) {
          ...
          {q.steps.map((s, i) => {
            ...
            return (
              <g
                ...
                onClick={() => { if (moved.current <= 6) navigate(`/mod/${modId}/steps/${s.id}`) }}
                onKeyDown={(e) => e.key === 'Enter' && navigate(`/mod/${modId}/steps/${s.id}`)}
                className="cursor-pointer outline-none [&:focus-visible>rect]:stroke-white"
                opacity={unreachable ? 0.45 : 1}
              >
                <rect width={NODE_W} height={NODE_H} rx={4} fill="var(--color-panel)" stroke={unreachable ? 'var(--color-dim)' : 'var(--color-edge)'} strokeDasharray={unreachable ? '5 4' : undefined} />
...
function DependsOn({ mod }: { mod: QuestView }) {
  ...
  const nameOf = (id: number) => {
    ...
    return GAME_QUESTS.find((g) => g.id === id)?.name ?? (local ? local.meta.title : t('output.flUnknownQuest', { id }))
  }
...
export function FlowPage() {
  ...
  return (
    <Page>
      ...
      ) : tab === 'map' ? <QuestOverviewMap q={q} modId={modId} /> : (
        <>
