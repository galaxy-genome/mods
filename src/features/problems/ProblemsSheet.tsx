        ...
        <ul className="flex flex-col divide-y divide-edge rounded-[4px] border border-edge bg-panel">
          {list.map((p) => (
            <li key={p.id} className="flex items-center gap-1 pr-1">
              <button type="button" onClick={() => goTo(p)} className="flex min-h-14 min-w-0 flex-1 items-start gap-3 px-3 py-2.5 text-left hover:bg-white/[0.03]">
              ...
              {p.fix && <Button variant="secondary" size="sm" onClick={() => p.fix!.apply()}>{p.fix.label}</Button>}
              {p.severity === 'tip' && (
