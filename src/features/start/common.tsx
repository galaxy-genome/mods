import { Check, ClipboardPaste, FileUp, Globe, Sparkles, Star, WifiOff } from 'lucide-react'
...
function TypeCard({ icon, title, body, onClick, primary }: { icon: React.ReactNode; title: string; body: string; onClick: () => void; primary?: boolean }) {
  return (
    <button onClick={onClick} className={cn('flex min-h-[72px] items-start gap-3 rounded-[4px] border px-3 py-3 text-left transition-colors', primary ? 'border-cyan bg-cyan/[0.08] hover:bg-cyan/15' : 'border-edge bg-panel hover:border-cyan')}>
