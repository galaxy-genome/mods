          ...
          <RowGroup>
            {shown.map(({ article: a, snippet }) => (
              <div key={a.slug} aria-current={a.slug === active ? 'page' : undefined} className={cn(a.slug === active && 'bg-cyan/10 shadow-[inset_2px_0_0_var(--color-cyan)] [&_*]:text-cyan')}>
                <ListRow icon={<FileText />} title={a.title} subtitle={snippet} onClick={() => navigate(`/help/${a.slug}`)} />
              </div>
            ))}
            {terms.map((id) => <ListRow key={id} muted title={termLabel(id)} subtitle={termDefinition(id)} onClick={() => navigate('/help/glossary')} />)}
          </RowGroup>
        </>
      )}
    </>
