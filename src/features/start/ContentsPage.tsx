...
import { dataUrlBytes, pairTextures, partsOf } from '@/lib/mods'
...
import { addQuestPart, addStarsPart, addTextures, deleteMod, duplicateMod, removeCommunityEntry, removePart, setFavorite, updateMod, useEditor } from '@/store/editor'
...
export function ContentsPage({ mod }: { mod: Mod }) {
  ...
  const onTextures = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = [...(e.target.files ?? [])]
    e.target.value = ''
    if (!files.length) return
    const { textures, unpaired } = pairTextures(await Promise.all(files.map(async (f) => ({ name: f.name, bytes: new Uint8Array(await f.arrayBuffer()) }))))
    if (textures.length) {
      addTextures(id, textures)
      toast.success(t('start.texturesAdded', { count: textures.length }))
    }
    if (unpaired.length) toast.error(t('start.textureUnpaired', { names: unpaired.join(', ') }), { description: t('start.textureUnpairedHelp') })
    if (!textures.length && !unpaired.length) toast.error(t('start.noTextureFiles'), { description: t('start.noTextureFilesHelp') })
  }
      ...
      <main className="mx-auto flex max-w-[720px] flex-col gap-5 px-4 py-4 pb-12">
              ...
              {shownQuests.map(({ view: v, after, n, depth, tree, last }, i) => {
                ...
                return (
                  <li key={v.meta.id} className="relative flex items-center" style={{ paddingLeft: depth * 12 }}>
                    ...
                    <button onClick={() => navigate(`/mod/${v.meta.id}/overview`)} className="flex min-h-16 min-w-0 flex-1 items-center gap-3 py-2.5 pl-3 text-left hover:bg-white/[0.03]">
        ...
        <section className="flex flex-col gap-2">
          ...
          {starsView && mod.stars ? (
            <div className="flex items-center rounded-[4px] border border-edge bg-panel">
              <button onClick={() => navigate(`/mod/${starsView.meta.id}/overview`)} className="flex min-h-14 flex-1 items-center gap-3 px-3 text-left hover:bg-white/[0.03]">
        ...
        <section className="flex flex-col gap-2">
          <SectionLabel action={!readOnly && (
            <label className="flex h-9 cursor-pointer items-center gap-1.5 px-1 text-[13px] font-semibold text-cyan hover:text-white focus-within:outline focus-within:outline-2 focus-within:outline-cyan">
              <ImagePlus className="size-4" />{t('start.addTextures')}
              <input type="file" multiple accept=".png,.xml,image/png,text/xml" className="sr-only" onChange={onTextures} />
            </label>
          )}>{t('start.texturesHeading', { n: mod.textures.length })}</SectionLabel>
          {mod.textures.length === 0 ? (
            <p className="text-[13px] text-dim">{readOnly ? t('start.noneInMod') : t('start.texturesEmpty')}</p>
          ) : (
