import { BookOpen, Database, Download, FileClock, FileText, Globe, HardDrive, Info, OctagonAlert, RefreshCw, RotateCcw, Sparkles, Trash2 } from 'lucide-react'
  ...
  return (
    <div className="min-h-dvh bg-void">
      ...
      <main className="mx-auto flex max-w-[640px] flex-col gap-3 px-4 py-4 pb-12">
        ...
        <Card className="flex flex-col gap-3 p-3">
          ...
          <div className="flex items-center gap-3 border-t border-edge pt-3">
            ...
            {!storage.persisted && (
              <Button size="sm" variant="warning" onClick={async () => toast((await requestPersist()) ? t('start.modsKept') : t('start.persistRefused'))}>{t('start.keepSafeShort')}</Button>
            )}
          </div>
        ...
        <RowGroup>
          <div className="px-3 py-1"><SwitchRow label={t('start.simStorageFull')} help={t('start.simStorageFullHelp')} checked={storage.simulateQuota} onCheckedChange={simulateQuota} /></div>
        ...
        <SectionLabel className="mt-3">{t('start.referenceData')}</SectionLabel>
        <RowGroup>
          <ListRow icon={<BookOpen />} title={t('start.gameVersion')} value="1.6.12" chevron={false} trailing={
            <Button size="icon-sm" variant="ghost" aria-label={t('start.checkUpdate')} onClick={() => toast(settings.offline ? t('start.youreOffline') : t('start.upToDate'), { description: settings.offline ? t('start.needsConnection') : t('start.gameVersionIs', { version: '1.6.12' }) })}><RefreshCw className="size-4" /></Button>
          } />
        </RowGroup>
        ...
        <RowGroup>
          ...
          <ListRow icon={<FileText />} title={t('start.licences')} subtitle={t('start.licencesHelp')} chevron={false} />
        </RowGroup>
      </main>
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => { await clearAllData(); location.assign('/') }}
      />
      <ConfirmDialog
        open={resetOpen}
