  ...
  return (
    <div className="flex min-h-dvh flex-col bg-void">
      ...
      <div className="safe-bottom fixed inset-x-0 bottom-0 z-30 border-t border-edge bg-deep/95 backdrop-blur">
        <div className="mx-auto flex max-w-[720px] gap-2 px-4 py-3">
          <Button variant="secondary" size="lg" className="flex-1" onClick={() => navigate(`/built-in/${g.id - 100000}/overview`)}><Eye className="size-4" />{t('startLib.openInEditor')}</Button>
          <Button variant="solid" size="lg" className="flex-1" onClick={copy}><Copy className="size-4" />{t('startLib.makeCopy')}</Button>
        </div>
      </div>
    </div>
  )
