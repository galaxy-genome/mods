  ...
  return (
    <Page className="pb-28">
      ...
      {preview && step.dialogue.length > 0 ? (
        <GamePreview lines={step.dialogue} steps={quest.steps} contact={quest.settings.charName} />
            ...
            render={(line, handle, i) => (
              <SwipeRow actions={[{ label: t('common.delete'), icon: <Trash2 />, tone: 'danger', onAction: () => remove(line) }]}>
                <div className="flex items-start gap-2.5">
                  <LineBubble line={line} steps={quest.steps} contact={quest.settings.charName} showChoices onClick={() => navigate(`${here}/${line.id}`, { replace: true })} label={t('steps.editLineN', { n: i + 1 })} />
                  {handle}
                </div>
              </SwipeRow>
            )}
...
export function LineBubble({ line, steps, contact, showChoices, onClick, label, text }: {
  line: DialogLine; steps: Step[]; contact?: string; showChoices?: boolean; onClick?: () => void; label?: string; text?: string
}) {
  const t = useT()
  const body = (
    <>
      ...
      <span className="flex min-w-0 flex-1 flex-col gap-2 rounded-[4px] border border-edge bg-[rgba(8,18,34,.92)] px-3 py-2.5">
        <span className="flex items-center gap-1.5">
          <span className="truncate text-[13px] font-semibold text-white">{line.speaker || contact || t('steps.noSpeaker')}</span>
...
function GamePreview({ lines, steps, contact }: { lines: DialogLine[]; steps: Step[]; contact?: string }) {
  const t = useT()
  ...
  return (
    <div className="grid-texture flex flex-col gap-3 rounded-[4px] border border-grid-strong p-3">
      ...
      <div role="button" tabIndex={0} aria-label={t('steps.nextLine')} onClick={next} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') next() }} className="cursor-pointer">
        <LineBubble line={line} steps={steps} contact={contact} text={line.text.slice(0, chars)} />
      </div>
