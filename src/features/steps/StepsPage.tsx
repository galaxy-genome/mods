...
import { useListParam } from '@/components/pickers/common'
...
export function StepsPage() {
  ...
  const [selected, setSelected] = useListParam('step', quest?.steps.length ?? 0)
  const select = (i: number) => { if (i !== selected) setSelected(i) }
          ...
          {steps.map((s, i) => {
            ...
            return (
              <React.Fragment key={s.id}>
                ...
                <StepCard
                  step={s} index={i} steps={steps} problem={worst(s)} selected={i === selected} onSelect={() => select(i)}
...
function StepCard({ step, index, steps, problem, selected, onSelect, onOpen, onLongPress, onDuplicate, onDelete, onInsertBefore, onInsertAfter }: {
  step: Step; index: number; steps: Step[]; problem?: Problem; selected: boolean; onSelect: () => void
  ...
  React.useEffect(() => {
    if (!selected || !card.current) return
