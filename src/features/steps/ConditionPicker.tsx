...
export function ConditionPicker({ open, onOpenChange, title, value, onSelect, step, steps, nested }: ConditionPickerProps) {
  ...
  const row = (def: ConditionDef) => (
    <ListRow key={def.action} title={def.label} subtitle={<AdvancedKey k={def.action.endsWith('_') ? `${def.action}…` : def.action} />} onClick={() => choose(def)}
      ...
      <div className="flex flex-col gap-4 pt-1">
        ...
        {query ? (
          ...
          <>
            ...
            <RowGroup>
              {CATEGORY_INFO.map((c) => (
                <ListRow key={c.name} icon={ICONS[c.icon]} title={c.label} subtitle={c.hint}
    ...
    case 'station': {
      ...
      return (
        <Field label={t('steps.station')} help={t('steps.stationHelp')}>
          ...
          {pickButton(t('steps.chooseStation'), value === 'OWN' ? t('steps.ownStation') : value, 'station')}
      ...
      return (
        <>
          ...
          <Field label={t('steps.place')} help={t('steps.placeHelp')}>
            ...
            {pickButton(t('steps.chooseStation'), place !== 'player' && placeKind === 'station' ? place : '', 'station')}
