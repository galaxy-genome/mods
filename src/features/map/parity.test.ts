...
test('checks: a star on a published generated system removes one of its cell', () => {
  fixture.cells.forEach((c: { cx: number; cy: number }) => {
    ...
    // The game skips the first generated position within reach, which can precede the one the probe sits on.
    const removes = mapChecks([star], galaxy).find((x) => x.key === 'removes')
    const names = new Set(real.map((st) => st.name as string))
    assert.ok(removes?.vars.other.split(', ').some((n) => names.has(n)), `cell ${c.cx},${c.cy}`)
  })
})
