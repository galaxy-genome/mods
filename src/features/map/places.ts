...
/**
 * A step with no place of its own happens wherever the player already is: the place its predecessors leave the player
 * at, following placeless predecessors back to the nearest placed step. It takes that place only when every path into it
 * arrives from the same place; from different places, or from nowhere, it stays placeless.
 */
export function inheritPlaces(q: QuestContent, places: QuestPlaces): QuestPlaces {
  const { edges } = stepGraph(q.steps)
  const at = (i: number) => places.steps[i].at(-1)
  // A step naming a system or station the map cannot find is somewhere, just not anywhere drawn. An unfound planet is in
  // the system the player is already in.
  const unknown = new Set(places.unresolved.filter((u) => u.place.kind !== 'planet').map((u) => u.step))
  const steps = places.steps.map((list, i) => {
    if (list.length || unknown.has(i)) return list
    ...
    while (stack.length) {
      const n = stack.pop()!
      const preds = edges.filter((e) => e.to === n).map((e) => e.from)
      if (!preds.length) lost = true
      for (const p of preds) {
        const place = at(p)
        if (unknown.has(p) && !place) lost = true
        else if (place) found.set(`${place.kind === 'area' ? 'area:' : ''}${place.x},${place.y}`, place)
        else if (!seen.has(p)) { seen.add(p); stack.push(p) }
      }
    }
    return !lost && found.size === 1 ? [...found.values()] : []
  })
  return { ...places, steps }
}
...
/** Eight-point direction of a map point from Sol; +Y is north. Null at Sol. */
export const compassOf = (p: { x: number; y: number }): Compass | null =>
  p.x || p.y ? COMPASS[(Math.round(Math.atan2(p.x, p.y) / (Math.PI / 4)) + 8) % 8] : null
...
export interface PlaceContext {
  ...
  /** The nearest other reachable catalogue systems, nearest first. */
  nearest: { name: string; x: number; y: number; ly: number }[]
}

/** Where a station or system sits: its system, distance and direction from Sol, and its neighbours. Null when the name does not resolve. */
export function placeContext(galaxy: Galaxy, resolve: Resolver, kind: 'system' | 'station', name: string, count = 3): PlaceContext | null {
  const at = kind === 'system' ? resolve.system(name) : resolve.station(name)
  if (!at) return null
  // ponytail: linear scan of the catalogue per call, index by cell if lists ever call this per row
  const nearest = !count ? [] : galaxy.reachable.filter((r) => r[0] !== at.system)
