...
export interface Label { px: number; py: number; rank: number; key: string; text: string; colour: string; always?: boolean; leftOf?: number }
...
export class Labels {
  ...
  draw(ctx: CanvasRenderingContext2D, c: Camera) {
    ...
    ctx.globalAlpha = 1
    ctx.font = '11px "JetBrains Mono", monospace'
    for (const { px: x0, py, rank, key, text, colour, always, leftOf } of this.list) {
