import { AlertTriangle, ChevronLeft, ChevronRight, Eye, EyeOff, Map as MapIcon } from 'lucide-react'
...
/** Every favorite stars mod's stars, drawn read-only. */
function useFavoriteStars(): Star[] {
  const parts = useEditor((s) => s.parts)
  return React.useMemo(() => parts.filter((m): m is StarsView => m.meta.type === 'stars' && m.meta.favorite).flatMap((m) => m.stars), [parts])
}
...
export function QuestOverviewMap({ q, modId, selected, onSelect }: { q: QuestContent; modId: string; selected: number | null; onSelect: (i: number | null) => void }) {
  const t = useT()
  const navigate = useNavigate()
  const resolve = useResolver(splitViewId(modId).modId)
  const favoriteStars = useFavoriteStars()

  const model = React.useMemo(() => {
    if (!resolve) return null
    const places = inheritPlaces(q, questPlaces(q, resolve))
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="relative">
        <StarMap
          ...
          mode="view"
          stars={NO_STARS}
          otherStars={favoriteStars}
          fit={fit}
          overlay={overlay}
          selectedPin={selected === null ? null : pinOfStep(selected)}
          focus={at && { x: at.x, y: at.y, seq: selected ?? 0 }}
          onPin={onPin}
          onStep={setSelected}
        />
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-void">
      ...
      {input.length === 0 ? (
        ...
        <>
          <div className="relative">
            <StarMap className="h-[58dvh] min-h-[300px] border-b border-edge" mode="view" stars={NO_STARS} otherStars={favoriteStars} fit={fit} overlay={model?.overlay} selectedPin={selectedPin} onPin={setSelectedPin} />
            {pin && series && (
