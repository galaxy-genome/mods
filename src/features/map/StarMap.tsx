...
import { DOT_R, INK, Labels, dotSm, drawGrid, drawOutline, drawSystems, loadGalaxy, loadGeneration, useGalaxy } from './galaxy'
import { type GeneratedStar, Generator, mix } from './generator'
import { PIN_R, fanPins, fitView } from './declutter'
...
export interface OverlayLine { x1: number; y1: number; x2: number; y2: number; colour: string; width?: number; dash?: boolean; arrow?: boolean; bend?: number; label?: string; dim?: boolean }
...
export interface StarMapProps {
  ...
  /** Where the view starts, and flies to when it changes. */
  focus?: { x: number; y: number; ly: number }
  /** Points and circles the view frames instead of `focus`: on first draw, when they change, and from the Fit button. */
  fit?: { x: number; y: number; r?: number }[]
  problems?: MapProblem[]
  overlay?: MapOverlay
...
/** The galaxy map canvas: grid, outline, catalogue and generated systems, and the editor's layers for `mode`. */
export function StarMap(props: StarMapProps) {
  ...
  const s = React.useRef({
    ...
    pins: [] as { id: string; px: number; py: number }[],
  ...
  }, [galaxy, maps, props.stars, props.otherStars])

  React.useEffect(() => { loadGalaxy().catch(() => {}) }, [])
  React.useEffect(() => { s.preview = null; invalidate() }, [props.stars, props.point, props.circle, props.problems, props.mode, props.overlay, props.selectedPin, invalidate, s])
  ...
  const focusKey = props.focus ? `${props.focus.x},${props.focus.y},${props.focus.ly}` : ''
  React.useEffect(() => {
    const f = propsRef.current.focus
    if (!f || !s.cam) return
    const scale = clampScale(s.cam.W, scaleFor(s.cam.W, f.ly))
  ...
  const fitTo = () => {
    const pts = propsRef.current.fit
    const v = pts && s.cam && fitView(pts, s.cam.W, s.cam.H)
    if (!v || !s.cam) return
  ...
  React.useLayoutEffect(() => {
    ...
    const resize = () => {
      ...
      if (!s.cam) {
        ...
        const v = propsRef.current.fit && fitView(propsRef.current.fit, W, H)
  ...
  function draw(ctx: CanvasRenderingContext2D, c: Camera) {
    ...
    const r = dotSm(c)
    const catalogue = drawSystems(ctx, c, s.catalogue, r)
  ...
  function drawOverlay(ctx: CanvasRenderingContext2D, c: Camera, o: MapOverlay, selectedPin: string | null | undefined) {
    ...
    for (const [i, { pin, w, px: tx, py: ty }] of placed.entries()) {
      ...
      if (pin.label && !pin.dim) s.labels.add({ px: px + w / 2 + 9, py, rank: on ? -3 : -2, key: `pin:${pin.id}`, text: pin.label, colour: on ? '#ffffff' : pin.start ? '#ffb454' : 'rgba(255,255,255,.85)', always: !pin.hollow, leftOf: px - w / 2 - 9 })
      if (!pin.dim) s.pins.push({ id: pin.id, px, py })
    }
    ctx.restore()
  }
  ...
  const onPointerEnd = (e: React.PointerEvent) => {
    ...
    if (p.overlay && p.onPin) {
      ...
      for (const pin of s.pins) { const d = (pin.px - at.x) ** 2 + (pin.py - at.y) ** 2; if (d < bd) { bd = d; best = pin.id } }
      p.onPin(best)
