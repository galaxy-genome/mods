...
test('checks flag outside, overlap, hides and moved catalogue systems', () => {
  ...
  assert.ok(ids.includes('soverlap-b:warning'), 'Sol is 1.4 ly away')
  assert.ok(ids.includes('shides-c:tip'))
  ...
  const hides = mapChecks([stars[2]], galaxy).find((c) => c.key === 'hides')!
  assert.equal(hides.vars.other, target.name)
  assert.equal(mapChecks([stars[4]], galaxy).filter((c) => c.key !== 'outside').length, 0)
})

test('two mod stars on top of each other overlap', () => {
  const ids = mapChecks([star('a', 'One', 5000, 5000), star('b', 'Two', 5002, 5001)], galaxy).map((c) => c.id)
  assert.ok(ids.includes('soverlap-a') && ids.includes('soverlap-b'))
})
