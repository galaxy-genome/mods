      ...
      {mode !== 'pick' ? (
        <div className="flex flex-col gap-2 border-t border-edge bg-deep px-3 pb-[calc(8px+env(safe-area-inset-bottom))] pt-2">
          <div className="flex gap-2">
            ...
            {mode === 'circle' && <label className="flex min-w-0 flex-1 flex-col gap-1 text-[12px] text-dim">{t('map.radius')}<NumberInput aria-label={t('map.radius')} value={radius} onChange={(r) => setRadius(Math.max(0, r))} /></label>}
          </div>
