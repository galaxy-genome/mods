...
import { GAME_QUESTS, LANGS, LICENCES, MOD_TAGS, PORTRAITS, QUEST_FACTIONS, STATIONS } from '@/lib/reference'
...
export function OverviewPage() {
  ...
  const versionValid = /^\d+\.\d+\.\d+$/.test(owner.meta.version)
      ...
      <div>
        ...
        {s.startMode !== 'space' && <Sec title={t('overview.sectionContact')} summary={s.charName ? `${s.charName} · ${s.charImage}` : t('overview.noContactName')}>
          ...
          <Field label={t('overview.portrait')} fieldKey="charImage" advancedKey="CharImage" help={t('overview.portraitHelp')} warning={!PORTRAITS.includes(s.charImage) ? t('overview.portraitUnknown', { name: s.charImage }) : undefined}>
            <button type="button" onClick={() => setPortraitOpen(true)} className="flex min-h-14 items-center gap-3 rounded-[2px] border border-edge bg-field px-2 py-1.5 text-left hover:border-cyan">
              <PortraitTile name={s.charImage} />
              <span className="flex-1 font-mono text-[15px] text-white">{s.charImage}</span>
              <ChevronRight className="size-4 text-dim" />
            </button>
          </Field>
          <div className="flex flex-col gap-1.5">
            <span className="section-label">{t('overview.barPreview')}</span>
            <Card className="grid-texture flex gap-3 p-3">
          ...
          {s.startMode === 'bar' && (
            <Field label={t('overview.station')} fieldKey="stationName" advancedKey="StationName" help={t('overview.stationHelp')} learnMore="/help/stars-and-stations"
              ...
              <SentenceCard icon={<Store className={s.stationName ? 'text-cyan' : 'text-danger'} />} tone={!s.stationName ? 'danger' : !stationKnown ? 'amber' : undefined} onClick={() => setStationOpen(true)} trailing={<ChevronRight className="size-4 text-dim" />}>
                {s.stationName ? <span className="font-mono">{s.stationName}</span> : <span className="text-dim">{t('overview.chooseStation')}</span>}
              </SentenceCard>
              {stationGuess && (
        ...
        <Section title={t('overview.sectionAbout')} defaultOpen={!versionValid} summary={[owner.meta.author || t('overview.noAuthor'), `v${owner.meta.version}`, owner.meta.licence].join(' · ')}>
          ...
          <fieldset disabled={readOnly} className="flex min-w-0 flex-col gap-5 disabled:opacity-60 disabled:[&_*]:pointer-events-none">
            ...
            <Field label={t('overview.version')} htmlFor="version" fieldKey="version" help={t('overview.versionHelp')} warning={!versionValid ? t('overview.versionInvalid') : undefined}>
              <Input id="version" value={owner.meta.version} warn={!versionValid} onChange={(e) => meta((m) => { m.version = e.target.value })} />
            </Field>
