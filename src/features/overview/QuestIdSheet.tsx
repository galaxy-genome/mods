...
import { GAME_QUEST_NAMES, gameQuest, GAME_QUEST_ID_MAX, GAME_QUEST_ID_MIN } from '@/lib/reference'
...
export function QuestIdSheet({ modId, open, onOpenChange }: { modId: string; open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  const taken = () => [...GAME_QUEST_NAMES.map((g) => g.id), ...parts.map(idOf).filter((x): x is number => x !== undefined)]
