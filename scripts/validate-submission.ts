...
//     reads the issue from a GitHub event, downloads its last attached zip, and with --write adds src/data/library/<id>.json
...
export async function validateZip(buf: Uint8Array, library: CommunityEntry[] = COMMUNITY): Promise<Result> {
  ...
  if (library.some((e) => e.id === id) || existsSync(new URL(`src/data/library/${id}.json`, ROOT))) errors.push(`The library already has \`${id}\`. Raise the mod's version for a new release.`)
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (write && result.entry && issue) {
    ...
    writeFileSync(new URL(`src/data/library/${entry.id}.json`, ROOT), JSON.stringify(entry, null, 2) + '\n')
