...
Format and game-behaviour facts it relies on are in `docs/ignored/`:
[mod-json-formats.md](../ignored/mod-json-formats.md), [mod-loading-rules.md](../ignored/mod-loading-rules.md),
[whats-new-since-2022.md](../ignored/whats-new-since-2022.md), [quest-tables-sqlite.md](../ignored/quest-tables-sqlite.md).
...
2. **Template sheet** (quests), each with a one-line description and a tiny step diagram:
   ...
   - *Delivery*: talk, fly to a system, dock, done.
...
The catalogue (`CONDITIONS` in `src/lib/conditions.ts`) holds every string the game dispatches as a button event while a quest runs, and nothing else: completion needs an exact match, so a string outside it never fires. `src/lib/conditions.test.ts` checks each entry against the decompiled `ButtonNames.as` and its dispatch sites. There is no free-text entry; an imported unknown action is a warning with fix **Choose a condition**, and the Advanced JSON view still shows raw values.
